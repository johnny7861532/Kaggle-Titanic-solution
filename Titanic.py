...
fill_value = {'Age':int(totall_data['Age'].median()),
              "Fare" : int(totall_data['Fare'].mode())
              , "Embarked" : "S"}
...
cm = confusion_matrix(np.float32(y_test), predict)
...
#submit to kaggle
test_data = pd.read_csv('test.csv',engine='python')
test_data = test_data.fillna(fill_value)
test_data['Family'] = test_data['SibSp']+ test_data['Parch']
reslut = test_data.drop(labels = ['PassengerId','Name','Ticket','Cabin','SibSp','Parch'],axis = 1)
